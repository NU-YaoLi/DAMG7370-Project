...
df_raw = spark.read.csv(f"s3://{BUCKET_NAME}/raw/*zori*.csv", header=True, inferSchema=True)
...
stack_expr = f"stack({len(date_cols)}, " + ", ".join([f"'{c}', `{c}`" for c in date_cols]) + ") as (date_str, rental_income)"
...
#  .parquet(f"s3://{BUCKET_NAME}/processed/rental_income/"))

(df_transformed.write.format("jdbc")
 .option("url", f"jdbc:postgresql://{DB_ENDPOINT}:5432/postgres")
 .option("dbtable", "landing.lnd_rental_income") # UPDATED to staging prefix
